      ...
      <div className="card">
        ...
        {!loading && results.length > 0 && (
          <table className="w-full border-collapse">
            ...
            <tbody>
              {results.slice((currentPage - 1) * pageSize, currentPage * pageSize).map((r, displayIdx) => {
                const absoluteIdx = (currentPage - 1) * pageSize + displayIdx;
                return (
                  <tr key={absoluteIdx} className="border-b border-slate-100 hover:bg-slate-50 text-[13px]">
                    <td className="py-3 px-3">
                      <input type="checkbox" checked={selected.has(absoluteIdx)} disabled={r.already_saved} onChange={() => toggleSelect(absoluteIdx)} />
                    </td>
                    <td className="py-3 px-3">
                    ...
                    <td className="py-3 px-3">
                      {r.already_saved
                        ...
                        : <button className="btn btn-primary btn-sm" onClick={() => { setSelected(new Set([absoluteIdx])); saveSelected(); }}><i className="ti ti-bookmark"></i> Save</button>}
                    </td>
                  </tr>
                );
              })}
        ...
        {!loading && results.length > 0 && (
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
            <span className="text-[12px] text-slate-400">
              Showing {(currentPage - 1) * pageSize + 1} to {Math.min(currentPage * pageSize, results.length)} of {results.length} businesses
            </span>
            <div className="flex gap-2">
              <button
              ...
              <span className="text-[12px] font-semibold flex items-center px-2 text-slate-500">
                Page {currentPage} of {Math.ceil(results.length / pageSize)}
              </span>
              <button
                className="btn btn-outline btn-sm"
                disabled={currentPage === Math.ceil(results.length / pageSize)}
                onClick={() => setCurrentPage(prev => Math.min(prev + 1, Math.ceil(results.length / pageSize)))}
              >
                Next
              </button>
            </div>
